 ...
 *
 * @author jiac
 * @version 2022.0.1 2022/4/28:18:34
 * @since 2022.0.1
 */
public abstract class ReactiveCrudRepositoryAdapter<T, ID, I extends CrudRepository<T, ID>>
    ...
    /**
     * deleteAll 的响应式适配器，由于 deleteAll(Iterable<? extends T> entities) 和 deleteAll(Publisher<? extends T> entityStream)
     * 具有不同的语义，我们无法将一个响应式调用直接映射到一个阻塞式调用。
     * deleteAll 方法订阅实体并为每个实体发出单独的 delegate.delete(entity) 请求，由于删除请求可以并行进行，因此每个请求都有自己的
     * subscribeOn 调用，从 scheduler 接收工作单元。
     * deleteAll 方法返回一个输出流，该输出流在传入流终止且所有删除惭怍完成时完成。
